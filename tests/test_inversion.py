...
def test_invert_mt1d_returns_result():
...
def test_inverted_model_structure():
    ...
    assert result.resistivity.shape == (3,)
    assert result.thickness.shape == (2,)


def test_calculated_response_shape():
    ...
    assert (
        result.rho_cal.shape
        == frequency.shape
    )

    assert (
        result.phase_cal.shape
        == frequency.shape
    )


def test_inversion_fitness_is_finite():
